...
test('presets/design.json の criteria が15件であること', () => {
  const preset = loadPreset(pluginRoot, 'design');
  assert.equal(preset.criteria.length, 15);
});
...
test('verification:auto の比率が design 20% / plan 50% / implement 78% であること', () => {
  ...
  assert.equal(autoRatio(design), 3 / 15);

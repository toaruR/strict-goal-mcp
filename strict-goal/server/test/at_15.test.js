...
function tmpDataDir() {
  return mkdtempSync(path.join(os.tmpdir(), 'strict-goal-at15-'));
}
...
function finalizeDesign(persistence, sessionId, content, criterionId, excerptOverride = undefined) {
...
test('AT-15: target_digest不一致/欠落はE_EVIDENCE_TARGET、テスト改変にdiffsが無ければE_TEST_MUTATED_WITHOUT_DIFF、非green時のauto満点はE_TEST_NOT_GREEN', () => {
  ...
  const planDigest = finalizeDesign(persistence, plan.session_id, PLAN_CONTENT, 'plan_works', PLAN_EXCERPT);

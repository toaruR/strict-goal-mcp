...
### SKILL.state 導入によるメリット:

長期エージェント実行基盤として **SKILL.state**（arXiv:2608.26263）のステートレス実行アーキテクチャを採用。サーバーがコンテキスト有界射影三つ組 $(P, \Sigma_t, O_t)$（不変仕様 $P$・標準状態 $\Sigma_t$・直近観測 $O_t$）を 4,000 文字以内で提供し、使い捨てサブエージェント（Ephemeral Workers: `sg-scout` / `sg-worker` / `sg-verifier`）が個別タスクをアトミックに処理します。

- **トークン消費の爆発防止**: 過去ログを引き回す $\mathcal{O}(T^2)$ の二次関数的増加を排除し、ステップあたり $\mathcal{O}(1)$、全体で $\mathcal{O}(T)$（線形）の低コスト運用を実現。
- **コンテキスト汚染と幻覚の排除**: 生のスタックトレースや試行錯誤履歴をサーバー側でサニタイズ・集約するため、コンテキストウィンドウの圧迫による注意散漫やルール忘れを防止。
- **完全な耐障害性・セッション復元**: 状態遷移と直近観測がサーバー側に記録されているため、長大セッションの中断・再開時も `loop_state` 1 回で必要最小限の状態へ即座に復帰可能。
...
## 人間フレンドリーな指示インターフェース (`/strict-goal` / 人語指示)
...
strict-goal のバージョンは以下のコマンドで確認できます:
...
- [SKILL.state: Scalable Long-Horizon Agent Skills (arXiv:2608.26263)](https://arxiv.org/abs/2608.26263?ref=globalfeed.ai) - ステートレス実行モデル、コンテキスト有界射影 $(P, \Sigma_t, O_t)$、および使い捨てワーカー（Ephemeral Workers）による長期エージェント実行アーキテクチャの理論的基盤。

...
import { computeManifestDigest } from './fileset.js';
...
export function saveFilesetArtifact(sDir, files, manifestCommand, manifestOutputSha256) {
  const digest = `sha256:${computeManifestDigest(files)}`;
  const hex = digest.slice('sha256:'.length);
  const target = path.join(artifactsDir(sDir), `sha256-${hex}.manifest.json`);
  fs.mkdirSync(artifactsDir(sDir), { recursive: true });
  const data = { files, manifest_command: manifestCommand, manifest_output_sha256: manifestOutputSha256 };
  writeJson(target, data);
  const bytes = Buffer.byteLength(JSON.stringify(files), 'utf8');
  return { digest, bytes };
}

export function readFilesetArtifact(sDir, digest) {

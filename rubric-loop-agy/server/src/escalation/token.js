...
export function currentEscalationId(sDir) {
  const nums = listEscalationNumbers(sDir);
  if (nums.length === 0) return null;
  return `esc_${String(Math.max(...nums)).padStart(2, '0')}`;
}

export function pendingEscalationId(sDir) {
  const nums = listEscalationNumbers(sDir);
  if (nums.length === 0) return null;
  const latestId = `esc_${String(Math.max(...nums)).padStart(2, '0')}`;
  const p = jsonPath(sDir, latestId);
  if (!fs.existsSync(p)) return null;
  const record = readJson(p);
  if (record && !record.consumed && fs.existsSync(tokenPath(sDir, latestId))) {
    return latestId;
  }
  return null;
}
...
export function createEscalation(sDir, { reason, summaryForHuman }) {
  const dir = escalationsDir(sDir);

...
function buildKickbackMustFix(upstreamSDir, upstreamRubricVersion, targetCriteria, note, fromSessionId) {
...
export function performKickback(dataDir, session, { targetCriteria, note }) {
  ...
  const kickbackMustFix = buildKickbackMustFix(
    upstreamSDir,
  ...
  if (upstreamSession.last_evaluation) {
    upstreamSession.last_evaluation.must_fix = kickbackMustFix;
  } else {
    upstreamSession.last_evaluation = { must_fix: kickbackMustFix };
  }
  ...
  session.freeze_reason = 'kicked_back';

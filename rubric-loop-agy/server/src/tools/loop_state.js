...
import { readArtifactContent, readFilesetArtifact } from '../artifact/store.js';
import { readTestInventory } from '../implement/test_inventory.js';
...
function readHistory(sDir) {
  ...
  for (const round of roundNumbers) {
    const file = path.join(roundsDir, `${round}.json`);
    if (fs.existsSync(file)) {
      try {
        ...
        // ignore malformed or partially written file
      }
    }
  }
  return records;
...
function buildUpstreamArtifact(dataDir, session, warnings) {
  ...
  let content;
  try {
    content = readArtifactContent(upstreamSDir, pinnedDigest, upstreamSession.artifact_kind);
  } catch {
    content = undefined;
  }
  return {
...
function buildChainSummary(dataDir, session) {
  ...
  return {
    ...
    links: perSession.map((entry) => ({
      ...
      state: readSession(dataDir, entry.session_id)?.state ?? 'MISSING',
      rounds: entry.round,
    })),
...
export function loopState({ input, persistence }) {
  ...
  if (include.includes('artifact_head') && session.current_artifact) {
    try {
      const rawContent = readArtifactContent(sDir, session.current_artifact.digest, session.artifact_kind);
      const limit = input.artifact_head_bytes ?? 2000;
      extra.currentArtifact = {
        ...session.current_artifact,
        head: typeof rawContent === 'string' ? rawContent.slice(0, limit) : JSON.stringify(rawContent).slice(0, limit),
      };
    } catch {
      extra.currentArtifact = session.current_artifact;
    }
  }
  if (include.includes('last_artifact') && session.current_artifact) {
    if (session.artifact_kind === 'fileset') {
      const manifest = readFilesetArtifact(sDir, session.current_artifact.digest);
      const testInventory = readTestInventory(sDir, session.round);
      extra.lastArtifact = {
        digest: session.current_artifact.digest,
        files: manifest?.files ?? [],
        ...(testInventory ? { test_inventory: testInventory } : {}),
      };
    } else {
      const rawContent = readArtifactContent(sDir, session.current_artifact.digest, session.artifact_kind);
      extra.lastArtifact = {

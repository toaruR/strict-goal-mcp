...
const PLAN_SUMMARY = 'これは40文字以上の長さを確実に満たす実装計画全体の概要説明文です。十分な文字数を確保しています。';
  ...
  tasks: [
    {
      ...
      acceptance: ['受け入れ条件が満たされること10文字以上'],
      verify: [{ command: 'npm test', expect_exit_code: 0 }],
    },
  ],
});

function commit(persistence, sessionId, expectedRound, content = CONTENT) {
...
function scoreOnce(persistence, sessionId, expectedRound, digest, scoreValue, excerpt = EXCERPT) {
  return scoreSubmit({
    input: {
      session_id: sessionId,
      submission_id: submissionId(),
...
test('plan モードでは改善のない周が2周連続すると STALLED になる', () => {
  ...
  let result;
  for (let i = 0; i < 4; i += 1) {
    const committed = commit(persistence, created.session_id, round, PLAN_CONTENT);
    result = scoreOnce(persistence, created.session_id, round, committed.artifact.digest, 5, PLAN_SUMMARY);
    if (result.verdict !== 'ITERATING') break;
    round = result.round;
  }
  assert.equal(result.verdict, 'STALLED');

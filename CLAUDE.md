...
- **Antigravity でワークスペース切り替え時に MCP サーバープロセスが残留する罠（恒久対策済み）**: 別プロジェクトを開いた後に本プロジェクトを開くと、IDE基盤が既存の MCP 子プロセス（`strict-goal` 等）を常駐・再利用し、起動引数 `--data-dir` が前プロジェクトのままになることがある。この問題は、`loop_open` 呼び出し時に `workspace_dir`（自ワークスペースのルートパス）を渡すことで、サーバー内の動的セッションレジストリ（`session_registry.js`）が自ワークスペースの `.strict-goal/` 配下に自動ルーティングして恒久解決された。後続の `artifact_commit` や `score_submit` 等もセッションIDから自動的に対象ワークスペースへ保存される。
...
- **`node --test`配下から子プロセスで`node --test`を叩くと`NODE_TEST_CONTEXT`環境変数の継承で"再帰呼び出し"と誤検知されexit 0でスキップされる罠(Linuxで顕著、CIのみ赤くなり手元Windowsでは気づけない)**: `helper.js`の`runCommand`がシェル経由でspawnする際`process.env`をそのまま渡すと、自分自身が`node --test`配下で実行されているケース（`at_ss.test.js`のような入れ子テスト）で子の`node --test`がスキップされ偽陽性のexit 0を返す。`spawnSync`に渡す`env`から`NODE_TEST_CONTEXT`を除去すること。Windows(cmd.exe)ではこの誤検知が起きず別経路（パスのクォート崩れ）でたまたま短いエラーになり検証をすり抜けていたため、再現確認は`wsl -e bash -lc "..."`でLinux実行環境を使うこと。
- **TAP出力の`# fail 1`等の要約行が正規表現`FAIL`に部分一致し偽の失敗エントリを生む罠**: `helper.js`の`parseTestOutput`と`sanitize_test.js`の`extractFailures`はどちらも`(?:✖|not ok|FAIL)`を大文字小文字無視でマッチさせるため、TAPのコメント/要約行（`# fail 1`, `# pass 0`等、`#`始まり）まで失敗マーカーとして誤検出し、本来の1件の失敗が2件に分裂したりstdoutが肥大化する。`#`始まりの行は判定前にスキップすること（spec/dot reporterの`ℹ`行と同様の除外が必要）。
...
- **`rubric_preset:"design"` はハーネス設計専用の15基準であり、任意の設計課題に当てると成果物をスコープ外へ膨らませる**: `packaging_conformance` / `self_hosting` / `host_portability` / `auditability` / `verdict_ownership` / `state_externalized` / `anti_gaming` は「エージェントハーネスの設計」を前提にしたアンカーを持つ。合否は全基準9点以上の最小値ゲート（`judge/engine.js:25`）なので、Rate Limiter クラスの設計のような課題でも「配布パッケージ」「自己適用レビュー記録」の節を書かないと合格できない（実測: `.benchmark/sandboxes/tr_0Q7S7P9CX2CDHW8Y51YEM6VXKF/`）。ハーネス以外の設計には `rubric` を明示指定するか、`docs/plans/design-rubric-scope-and-verification.md` の改善1（プリセット分割）を適用すること。
- **散文の「優先順位」はこのハーネスでは一切効かない**: 判定は最小値ゲート（全基準が同格）で、`must_fix` も `score_submit.js:81` の `buildMustFix` がスコア昇順のみで `MUST_FIX_MAX=3` に切る。優先順位を効かせたいなら基準に `priority` を持たせて `buildMustFix` のソートキーにする（同設計書の改善7）。
...
- **`artifact_commit` の `source_path` は宣言だけで実装が読まない（ファイル渡し不可）**: `schemas/tools.json:388` に `source_path`（string, maxLength 4096）があるが `src/tools/artifact_commit.js` は一切参照しない（`grep -c source_path src/` = 0）。markdown/plan の成果物はインライン `content` 必須で、`files` を渡すと `E_ARTIFACT_KIND_MISMATCH`。よって数万バイトの成果物のコミットを `sg-verifier` 等のサブエージェントへ委譲できず、作者側で全文を `content` に載せるしかない（採点のみ委譲する運用にする）。
- **`content` で渡した成果物は末尾改行が1つ増えて保存されることがある（digest がディスクのファイルと一致しない）**: `saveContentArtifact` は受け取った `content` をそのまま sha256 するため、ツール引数の受け渡し経路で末尾に `\n` が付くと digest がずれる（実測: ディスク 49898B の JSON を渡して 49899B / 別 digest が返った）。byte 一致を確認するときは `printf '\n' | cat file - | sha256sum` と比較し、一致したら本文は同一。以後の locator 根拠をディスク本文から切り出す運用なら、ファイル側に末尾改行を足して digest を揃えておくこと。
- **`criteria` の新フィールド（`priority` 等）は `from_input.js` で明示的に配線しないと永続化時に脱落する**: `schema.js` と `tools.json` にプロパティを追加しても、入力形から永続形へ変換する `convertInputCriterion`（`src/rubric/from_input.js`）で `priority: criterion.priority ?? 0` を明示的にコピーしないと、セッション保存・復帰時にフィールドが削ぎ落とされる。
- **プリセット基準数やポリシーの改訂は `presets.test.js` などの既存件数アサーションと直接連動する**: `presets/design.json` などの基準数・基準ID・ポリシー値（`artifact_budget_bytes` 等）を変更すると、`test/presets.test.js` や `test/defaults_table.test.js`, `test/loop_open_create.test.js` にある厳密な基準数・スナップショット照合が連動して失敗するため、プリセット更新時はテスト側のアサーションもセットで更新する必要がある。
- **ホスト標準のスラッシュコマンドと同名のスキル（`skills/goal`）を置くと、ホスト機能が strict-goal にハイジャックされる罠**: Antigravity や Claude Code 等はプロジェクト内の `.agents/skills/` や `.claude/skills/` を優先読み込みするため、`goal` という名前のスキルを定義すると標準の `/goal` コマンドがすべて `strict-goal` 起動に奪われてしまう。strict-goal の起動は `/strict-goal` や `strict-goal` プレフィックスに限定し、ホスト標準コマンドと競合する名前のスキル定義は避けること。
...
- **CI環境で外部CLI（agy/claude/codex）不在時のinvoke_subagentテストは一時モックで隔離する**: GitHub Actions等のクリーンなrunnerには各エージェントCLIが存在しないため、`invoke_subagent` は `E_RUNNER_NOT_FOUND` を投げる。テストでは一時ディレクトリにモックスクリプトを配置してPATH先頭に追加することで、CI上でも外部依存なしに正常系を検証可能。
- **log_store のログローテーション（mtimeMsソート）は同一ミリ秒書き込み時のタイブレークが必須**: ループ内で高速に連続保存した場合、ファイルシステムのmtime解像度により同一ミリ秒となりソート順が不安定化し、最新のログが誤ってローテーション削除される。`mtimeMs` が同一の場合はファイル名降順（`localeCompare`）でタイブレークすること。
...
- **ベンチマーク `strict_hierarchical` 群の Claude 実行は実際には単一エージェントで走っていた**: `run-agent-benchmark.js` はサブエージェント定義を `.agents/agents/` にしか複製せず、Claude Code は `.claude/agents/` しか読まないため `sg-verifier` 等が一覧に現れない（`.codex/agents/*.toml` も未複製だった。いずれも `run-agent-benchmark.js` で `.claude/agents/`・`.agents/agents/`・`.codex/agents/` の3箇所へ複製するよう修正済み）（`~/.claude/projects/<sandbox>/…jsonl` の `agent_listing_delta` で確認可）。結果、著者が自己採点・全文 `Write`・全文インライン `artifact_commit` を繰り返しコンテキスト 160K→自動コンパクションで成果物を失い 1800 秒タイムアウト（tr_0Q2T23FM1JT2KQDA6C6DWYNMS1: 89 API 呼び出し / cache_read 7.17M / output 214K）。またタイムアウト時もキーワード式 held-out テストが通るため `resolved: true` と記録され、報告上は合格に見える。Codex 側は子スレッドのトークンを集計していない（親 1.95M に対し子 4 本 ≈2.6M が未計上）。
- **Claude Code のセッション jsonl は content ブロックごとに 1 行で同一 `usage` を複写する**: 行単位で `usage` を合計すると 2〜3 倍に過大計上される（実測: 7.86M と報告された試行の実消費は 4.3M）。`message.id` で重複排除すること（`recalculate-benchmarks.js` の `sumClaudeJsonl` で対応済み）。サブエージェントの消費は `<session>/subagents/agent-*.jsonl` に別置きされ、メインの `usage` にも `claude -p` の最終 JSON にも含まれないので別途合算が必要。
- **`score_submit` の evidence 形式はスキーマ上 `{type:"object"}` のみで、`next_action.input_skeleton` が `{}` だったため、検証エージェントが毎回サーバソース／tests を grep して形式を逆引きし、1採点で 6 回 `E_VALIDATION` を往復していた（sg-verifier 1 本で 14 分・2.3M トークン）**: `envelope.js` が全 kind の記入例付きスケルトンを返すようにし、validate.js は欠落必須キーを一度に全部列挙するようにした。スキーマの `description` は validate.js の許容キーワードに追加済み。
- **ベンチマークのサンドボックスは `.benchmark/sandboxes/` 配下にあるため、Claude Code が親ディレクトリを遡ってルートの `CLAUDE.md`（本ファイル）を読み込む**: strict 群では複製版と合わせて二重に載り（初回コンテキスト 99K）、しかもハマりポイントの記述（「design preset は使わずカスタム rubric」等）が被験エージェントの方針を汚染して自己採点 9/9 の即 FINAL（tr_VFYE9HR3…）を招いた。対策済み: 複製を止め、サンドボックス既定位置を `<os.tmpdir()>/strict-goal-bench/<trial_id>`（リポジトリ外）へ移した（`--sandbox-root` で変更可。所在は `trials/<tid>/sandbox_path.txt` に記録）。実際に本注記を読んだ被験エージェントが「汚染に気づいた」と申告して作業前に停止した試行（`tr_GQPFBBR3…`）があり、開発メモが挙動を変える実害を確認した。

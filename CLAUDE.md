...
- **ベンチマークのサンドボックスは `.benchmark/sandboxes/` 配下にあるため、Claude Code が親ディレクトリを遡ってルートの `CLAUDE.md`（本ファイル）を読み込む**: strict 群では複製版と合わせて二重に載り（初回コンテキスト 99K）、しかもハマりポイントの記述（「design preset は使わずカスタム rubric」等）が被験エージェントの方針を汚染して自己採点 9/9 の即 FINAL（tr_VFYE9HR3…）を招いた。複製は止めたが遡り読込は残るので、根本対策はサンドボックスをリポジトリ外（例: `%TEMP%`）へ移すこと。
